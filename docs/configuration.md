...
| `TRITON_DEPLOY_CODE_SERVER_EXTENSION_DIR` | No | auto-detected | Optional source directory for the bundled code-server Triton Deploy extension. Set this for local backend runs if the repository extension directory is not found automatically. Use forward slashes in Windows `.env` files. |
