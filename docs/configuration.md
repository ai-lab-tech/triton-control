...
The Settings page separates **OIDC** and **Email** configuration into tabs.
`EMAIL_CONFIG_SOURCE` determines whether administrators can edit the Email tab:

| Value | Behavior                                                                                                                                                                                | Typical Use                                    |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `db`  | Email settings are stored in the database and can be edited and saved through the Email tab. Stored SMTP passwords are never returned; the UI only reports that a stored secret exists. | UI-managed deployments.                        |
| `env` | Email settings come from environment variables or mounted secrets. The Email tab displays the effective configuration as read-only.                                                     | Helm, Compose, and GitOps-managed deployments. |

In database-managed mode, selecting `disabled` disables SMTP, link-lifetime,
certificate, and message-content fields. The delivery-mode selector and save
action remain available so an administrator can re-enable the feature later.
Invitation, reissue, cancellation, and password-reset controls are unavailable
in User Management while delivery is disabled.

For SMTP, save the configuration before using **Send test email**. The test
action remains disabled while there are unsaved changes because the endpoint
tests the persisted configuration. SMTP tests, invitations, and administrator
password resets display progress while waiting for the bounded SMTP operation.
...
| `DEVELOPMENT_CODE_SERVER_VERSION` | No | `4.125.0` | Managed code-server runtime version. The native S3 Windows-style drag customization supports this pinned build and rejects incompatible workbench bundles. Custom images with code-server must provide the supported build and a writable bundle. |
...
| `EMAIL_RESET_SUBJECT`, `EMAIL_RESET_TEXT_TEMPLATE`, `EMAIL_RESET_HTML_TEMPLATE`    | No                                 | built in                                                     | Reset templates. Text and HTML templates must contain `{link}`. Supported placeholders are `{display_name}`, `{product_name}`, `{link}`, and `{expiry_minutes}`.                                                                                                                |
