"""Public benchmark command preparation and input conversion helpers."""
...
def perf_analyzer_namespace() -> str:
...
def run_command(
    ...
    *,
    ...
    if decoupled:
        ...
        protocol = "grpc"
    batch_size = effective_perf_analyzer_batch_size(request, decoupled=decoupled)
    cmd = [
        "perf_analyzer",
        "-m",
        request.model_name,
...
def effective_perf_analyzer_batch_size(request: RunPerfAnalyzerRequest, *, decoupled: bool) -> int:
    if decoupled:
        return 1
...
def _requires_decoupled_perf_analyzer_mode(
    ...
    *,
    ...
    """
    config = fetch_triton_model_config(instance, model_name=model_name, model_version=model_version)
    if model_config_requires_decoupled_perf_analyzer_mode(config):
        return True
...
def model_config_requires_decoupled_perf_analyzer_mode(config: dict[str, Any] | None) -> bool:
    if not config:
        return False
...
def prepare_input_data_for_perf_analyzer(input_data: str | None, *, decoupled: bool = False) -> str | None:
...
def direct_perf_input_argument(value: str) -> str | None:
    raw = (value or "").strip()
    if not raw:
        return None
    lower = raw.lower()
