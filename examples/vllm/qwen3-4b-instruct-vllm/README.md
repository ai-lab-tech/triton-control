# Qwen3 4B Instruct with Triton vLLM

General-purpose instruction LLM served through Triton's vLLM backend. Tuned for
a single GPU with up to 16 GB VRAM.
...
- Artifact directory: `qwen3_4b_instruct/1/model/`
- Config: `qwen3_4b_instruct/1/model.json`
...
```text
convert_qwen3_4b_instruct_to_vllm.ipynb
```
...
If deployment runs out of memory, reduce `max_model_len` to `4096`.
...
2. Select model `qwen3_4b_instruct`.
...
```bash
curl -X POST "http://localhost:8000/v2/models/qwen3_4b_instruct/generate" \
...
python infer_client.py --url localhost:8001 "Explain Triton Control in two sentences."
```
